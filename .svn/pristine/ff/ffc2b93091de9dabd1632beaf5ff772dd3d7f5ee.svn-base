...
////            gl.DeleteRenderbuffersEXT(2, new uint[] { colourRenderBufferID, depthRenderBufferID });
            GL.DeleteFramebuffers(2, new uint[] { colourRenderBufferID, depthRenderBufferID });
...
////			    gl.ReadPixels(0, 0, Width, Height, OpenGL.GL_BGRA, OpenGL.GL_UNSIGNED_BYTE, dibSection.Bits);
                GL.ReadPixels(0,0,Width, Height, PixelFormat.Bgra, PixelType.UnsignedByte, dibSection.Bits);

			    //	Blit the DC (containing the DIB section) to the target DC.
			    Win32.BitBlt(hdc, 0, 0, Width, Height, dibSectionDeviceContext, 0, 0, Win32.SRCCOPY);
		    }
        }
