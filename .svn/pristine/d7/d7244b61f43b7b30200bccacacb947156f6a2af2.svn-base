...
//**            RLog.info(Trans.T("L_ANALYSER_FINISHED"));
        ...
        public void CutMesh(Submesh mesh, RHVector3 normal, RHVector3 point,int defaultFaceColor)
        {
            ...
            bool drawEdges = SettingsProvider.Instance.ThreeDSettings.ShowEdges;
            foreach (TopoEdge e in edges)
        ...
        public void exportSTL(string filename, bool binary)
        {
            ...
            else
            {
                ...
                w.WriteLine("endsolid _3DPrinter");
                w.Close();
            }
            fs.Close();
        }
        ...
        public bool importObj(byte[] data, string filename,double scale=1)
        {
            clear();
            bool error = false;
            try
        ...
        private void importSTLAscii(byte[] data,string filename,double scale)
        {
//            string text = System.IO.File.ReadAllText(filename);
        ...
        public void importSTL(byte[] data,string filename,double scale=1)
        {
...
//                FileStream f = File.OpenRead(filename);
                ...
                if (stream.Length != 84 + nTri * 50)
                {
                    stream.Close();
                    importSTLAscii(data,filename,scale);
                }
                else
                {
...
//**                            Application.DoEvents();
            ...
            {
                MessageBox.Show(e.ToString(), "Error reading STL file");
            }
        }
        ...
        public bool import3Ds(byte[] data, string filename,double scale=1)
        {
