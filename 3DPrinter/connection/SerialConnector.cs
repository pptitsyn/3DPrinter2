...
namespace _3DPrinter.connection
{
    ...
    public class SerialConnector : PrinterConnectorBase, INotifyPropertyChanged, IDisposable
    {
     ...
     //   private SerialConnectionPanel panel = null;
        ...
        public SerialConnector(PrinterConnection conn)
        {
            Setup();
            con = conn;
            con.connector = this;
            job = new Printjob(conn);
        ...
        private void Setup()
        {
            BaudRate = ProjectManager.Instance.CurrentProject.projectSettings.PrinterSettings.BaudRate.ToString();
            Port = ProjectManager.Instance.CurrentProject.projectSettings.PrinterSettings.ComPortName;
            ReceiveCacheSizeString = ProjectManager.Instance.CurrentProject.projectSettings.PrinterSettings.ReceiveCacheSize.ToString();
            PingPong = ProjectManager.Instance.CurrentProject.projectSettings.PrinterSettings.PingPong;
            Protocol = ProjectManager.Instance.CurrentProject.projectSettings.PrinterSettings.TransferProtocol;
            ResetOnConnect = ProjectManager.Instance.CurrentProject.projectSettings.PrinterSettings.ResetConnect;
            ResetOnEmergency = ProjectManager.Instance.CurrentProject.projectSettings.PrinterSettings.ResetEmergency;
 ...
 //               Main.main.printerInformationsToolStripMenuItem.Enabled = false;
        ...
        public override void RunPeriodicalTasks()
        {
            ...
            else if (resendError < 4 && read.Length > 0 && lastReceived - actTime > 400)
            {
                ...
                con.log("L_RESET_OUTPUT", false, 2); // "Reset output. After some wait, I got only " + read
                read = "";
                if (pingPong)
...
//*/                Application.DoEvents();
                ...
                if (resetOnConnect == 0)
                {
                    ...
                    InjectManualCommand("T" + 0); //Main.main.printPanel.comboExtruder.SelectedIndex);
                ...
                {
                    garbageCleared = true;
                    con.FireConnectionChange("L_CONNECTED" + ":" + con.printerName);
                    Application.Current.MainWindow.Dispatcher.Invoke(con.Main.main.UpdateJobButtons);
...
//                    if (!Main.IsMono)
                ...
                con.FireConnectionChange("L_CONNECTION_ERROR" + ":" + con.printerName);
//                RepetierHost.view.SoundConfig.PlayError(false);
//                if (MessageBox.Show("L_CONNECTION_FAILED", "L_CONNECTION_ERROR", MessageBoxButtons.YesNo, MessageBoxIcon.Error) == DialogResult.Yes)
//                {
...
//                }
        ...
        public override bool Disconnect(bool force)
        {
            ...
            if (heateron && !force)
            {
                MessageBoxResult heatres = MessageBox.Show("L_HEATERS_ON_QUEST", "L_WARNING", MessageBoxButton.YesNoCancel, MessageBoxImage.Warning);
                if (heatres == MessageBoxResult.Cancel) return false;
                if (heatres == MessageBoxResult.Yes)
                {
...
//*/            Application.DoEvents();
        ...
        public override void InjectManualCommand(string command)
        {
            ...
            if (job.dataComplete == false)
            {
                ...
                {
                    con.firePrinterAction("L_X_COMMANDS_WAITING"+ injectCommands.Count.ToString());
                }
            }
        ...
        public override System.Windows.Controls.UserControl ConnectionDialog()
        {
//            if (panel == null)
//            {
...
//            return panel;
        ...
        public override void LoadFromRegistry()
        {
            ...
            ReceiveCacheSizeString = "127";// (string)key.GetValue("receiveCacheSize", receiveCacheSize.ToString());
        ...
        public override void RunJob()
        {
            ...
            List<GCodeShort> content = con.Main.main.codeEditor.getContentArray(0);
            if (content.Count == 0) return; // nothing to print
            if (content[0].text.StartsWith("@continuedScript"))
                isContinuedJob = true;
            job.BeginJob();
            if (!isContinuedJob)
                job.PushGCodeShortArray(con.Main.main.codeEditor.getContentArray(1));
            job.PushGCodeShortArray(content);
            if (!isContinuedJob)
                job.PushGCodeShortArray(con.Main.main.codeEditor.getContentArray(2));
            job.EndJob();
            Application.Current.MainWindow.Dispatcher.Invoke(con.Main.main.UpdateJobButtons);
        }
        ...
        public override void PauseJob(string text)
        {
            ...
            foreach (GCodeShort code in con.Main.main.codeEditor.getContentArray(4))
            {
        ...
        public override void ResendLine(int line)
        {
            ...
            lock (nextlineLock)
            {
                ...
                lock (history)
                {
                    ...
                    {
                        con.log("L_RECEIVING_ONLY_ERRORS", false, 2); // Receiving only error messages. Stopped communication.
                        con.close(true);
//                        RepetierHost.view.SoundConfig.PlayError(false);
        ...
        public bool TrySendNextLine2()
        {
            ...
            try
            {
                ...
                lock (nextlineLock)
                {
                    ...
                    try
                    {
                        ...
                        if (injectCommands.Count > 0 && !job.exclusive)
                        {
                            ...
                            if (job.dataComplete == false)
                            {
                                if (injectCommands.Count == 0)
                                {
                                    printeraction = "L_IDLE";
                                }
                                else
                                {
                                    printeraction = "L_X_COMMANDS_WAITING"+ injectCommands.Count.ToString();
                                }
                            }
                            return true;
                        }
                        ...
                        if (job.dataComplete && !paused)
                        {
                            ...
                            if (!gc.hostCommand)
                            {
                                ...
                                linesSend++;
                                lastCommandSend = DateTime.Now.Ticks;
                                printeraction = "L_PRINTING..ETA"+ job.ETA; //"Printing...ETA " + job.ETA;
                                if (job.maxLayer > 0)
                                    printeraction += " " +"L_LAYER_X/Y"+ con.analyzer.layer.ToString()+ job.maxLayer.ToString(); // Layer " + analyzer.layer + "/" + job.maxLayer;
                    ...
                    {
                        logtext = "L_ERROR_SENDING_DATA:" + ex; // "Error sending data:" + ex;
                        loglevel = 2;
                    }
                }
            }
            finally
            {
                ...
                // the lock statement.
                if (hostCommand != null)
                    Application.Current.MainWindow.Dispatcher.Invoke(con.Main.main.executeHostCall, hostCommand, con);
                if (historygc != null)
        ...
        {
            comErrorsReceived++;
            con.log("L_SERIAL_COM_ERROR" + e.ToString(), false, 2); // "Serial com error:"
