...
namespace _3DPrinter.view.ThreeD
{
    public class ThreeDCamera
    {
        ...
        public double phi = 0;
        public double angle = 45.0 * Math.PI / 180;
        //**        FormPrinterSettings ps = SettingsProvider.Instance.Printer_Settings;
        ...
        public void Zoom(double dis)
        {
            ...
            if (distance > 5 * defaultDistance)
                distance = 5 * defaultDistance;
            if (distance > 1)
            {
        ...
        public void FitSelectedObject(PrintModel model)
        {
            FitBoundingBox(model.bbox);
        }
        ...
        public void FitBoundingBox(RHBoundingBox box)
        {
            ...
            while (loops > 0)
            {
                loops--;
                angle = 15.0 * Math.PI / 180;
                double ratio = (double)control.view_Container.ActualWidth / (double)control.view_Container.ActualHeight;
