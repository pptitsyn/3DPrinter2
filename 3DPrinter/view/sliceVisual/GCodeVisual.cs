...
namespace _3DPrinter.view.sliceVisual
{
      ...
      //  public static bool correctNorms = true; // Draw correct normals
        ...
        public void UpdateVBO(bool buffer)
        {
            ...
            if (hasBuf)
                GL.DeleteBuffers(3, buf);
            hasBuf = false;
            int method = SettingsProvider.Instance.ThreeDSettings.filamentVisualization;
            float h = SettingsProvider.Instance.ThreeDSettings.layerHeight;
            float wfac = SettingsProvider.Instance.ThreeDSettings.widthOverHeight;
            float w = h * wfac;
            bool fixedH = SettingsProvider.Instance.ThreeDSettings.useLayerHeight;
            float dfac = (float)(Math.PI * SettingsProvider.Instance.ThreeDSettings.filamentDiameter * SettingsProvider.Instance.ThreeDSettings.filamentDiameter * 0.25 / wfac);
            int nv = 8 * (method - 1), i;
            if (method == 1) nv = 4;
            if (method == 0) nv = 1;
            ...
            if (method != 0) positions = new float[pointsCount * nv * 3 * (SettingsProvider.Instance.ThreeDSettings.CorrectNorms ? 2 : 1)]; else positions = new float[3 * pointsCount];
            if (method != 0) normals = new float[pointsCount * nv * 3 * (SettingsProvider.Instance.ThreeDSettings.CorrectNorms ? 2 : 1)]; else normals = null;
            ...
            if (method > 0)
            {
                ...
                foreach (LinkedList<GCodePoint> points in pointsLists)
                {
                    ...
                    while (ptNode != null)
                    {
                        ...
                        alpha = 0;
                        float c, s;
                        int b = vpos / 3 - nv * (SettingsProvider.Instance.ThreeDSettings.CorrectNorms ? 2 : 1);
                        for (i = 0; i < nv; i++)
                        {
                            c = (float)Math.Cos(alpha) * dh;
                            s = (float)Math.Sin(alpha) * dw / zoomw;
                            if (SettingsProvider.Instance.ThreeDSettings.CorrectNorms)
                            {
                            ...
                            if (SettingsProvider.Instance.ThreeDSettings.CorrectNorms && ptNode == null)
                            {
                                normals[npos++] = actdir[0];
                                normals[npos++] = actdir[1];
                                normals[npos++] = actdir[2];
                        ...
                        {
                            b = vpos / 3 - nv * (SettingsProvider.Instance.ThreeDSettings.CorrectNorms ? 2 : 1);
                            int nn = (nv - 2) / 2;
                            for (i = 0; i < nn; i++)
                            {
                                if (SettingsProvider.Instance.ThreeDSettings.CorrectNorms)
    ...
    public class GCodeVisual : ThreeDModel
    {
        ...
        public GCodeVisual()
        {
            MaxExtruder = SettingsProvider.Instance.SelectedPrinterSettings.numExtruder;
            segments = new LinkedList<GCodePath>[MaxExtruder];
            for (int i = 0; i < MaxExtruder; i++)
                segments[i] = new LinkedList<GCodePath>();
            ana = new GCodeAnalyzer(true);
            ana.GCodeVisual = this;
            startOnClear = true;
            ana.eventPosChanged += OnPosChange;
            ana.eventPosChangedFast += OnPosChangeFast;
        }

        public GCodeVisual(CodeEditor editor) 
            : this()
        {
            this.editor = editor;
        }
        ...
        public void Invoke_Method(Delegate methodDelegate, params object[] param)
        {
            if (OwnerControl != null)
            {
                OwnerControl.Dispatcher.Invoke(methodDelegate, param);
            }
            else
            {
                throw new Exception("OwnerControl is null!");
            }
        }
        ...
        {
            if (!liveView) return;
            if (SettingsProvider.Instance.ThreeDSettings.filamentVisualization < ana.maxDrawMethod)
                ana.maxDrawMethod = SettingsProvider.Instance.ThreeDSettings.filamentVisualization;
...
//**                Main.conn.log("Reduced visual quality for better framerates and to protect print quality.", false, 1);
        ...
        void OnPosChange(GCode act, float x, float y, float z)
        {
            ...
            bool isTravel = (SettingsProvider.Instance.SelectedPrinterSettings.printerType == 3 ? Math.Max(z, lastz) - ana.zOffset >= SettingsProvider.Instance.SelectedPrinterSettings.cncZTop : !ana.eChanged);
            ...
            if (isTravel)
            {
                ...
                if (SettingsProvider.Instance.SelectedPrinterSettings.printerType == 3 && z - ana.zOffset < SettingsProvider.Instance.SelectedPrinterSettings.cncZTop) // End od travel moves
            ...
            if (seg.Count == 0 || (laste >= ana.activeExtruder.e && SettingsProvider.Instance.SelectedPrinterSettings.printerType != 3)) // start new segment
            {
            ...
            {
                if (!isLastPos && (SettingsProvider.Instance.SelectedPrinterSettings.printerType != 3 || !isTravel))
                {
                    totalDist += locDist;
        ...
        void OnPosChangeFast(float x, float y, float z, float e)
        {
            ...
            if (lastLayer == minLayer - 1 && (laste < e || SettingsProvider.Instance.SelectedPrinterSettings.printerType == 3))
            {
                GCodePath p = new GCodePath();
        ...
        {
            if (SettingsProvider.Instance.ThreeDSettings.drawMethod == 2)
            {
                GL.Material(MaterialFace.FrontAndBack, MaterialParameter.AmbientAndDiffuse, defaultColor);
                ...
                if (liveView && path.lastDist > minHotDist)
                {
                    ...
                    int nv = 8 * (method - 1);
                    if (method == 1) nv = 4;
                    if (SettingsProvider.Instance.ThreeDSettings.CorrectNorms) nv *= 2;
                    if (method == 0) nv = 1;
                    int p = 0;
            ...
            else
            {
                ...
                if (SettingsProvider.Instance.ThreeDSettings.drawMethod > 0) // Is also fallback for vbos with dynamic colors
                {
                    GL.EnableClientState(ArrayCap.VertexArray);
                    if (path.elements == null) return;
        ...
        {
            if (SettingsProvider.Instance.ThreeDSettings.drawMethod != 2) return;
            ...
            Color col = SettingsProvider.Instance.ThreeDSettings.travelMoves;
        ...
        {
            changed = false;
            if (SettingsProvider.Instance.ThreeDSettings.drawMethod != 2 && colbufSize > 0)
            {
                GL.DeleteBuffers(1, colbuf);
                colbufSize = 0;
            }
            if (SettingsProvider.Instance.ThreeDSettings.checkDisableFilamentVisualization) return; // Disabled too much for card
            hotFilamentLength = SettingsProvider.Instance.ThreeDSettings.hotFilamentLength;
            minHotDist = totalDist - hotFilamentLength;
            Reduce(); // Minimize number of VBO
            //long timeStart = DateTime.Now.Ticks;
            Color col;
            col = SettingsProvider.Instance.ThreeDSettings.hotFilament;
            ...
            method = SettingsProvider.Instance.ThreeDSettings.filamentVisualization;
            if (method > ana.maxDrawMethod) method = ana.maxDrawMethod;
            wfac = SettingsProvider.Instance.ThreeDSettings.widthOverHeight;
            h = SettingsProvider.Instance.ThreeDSettings.layerHeight;
            w = h * wfac;
            fixedH = SettingsProvider.Instance.ThreeDSettings.useLayerHeight;
            dfac = (float)(Math.PI * SettingsProvider.Instance.ThreeDSettings.filamentDiameter * SettingsProvider.Instance.ThreeDSettings.filamentDiameter * 0.25 / wfac);
            recompute = lastFilHeight != h || lastFilWidth != w || fixedH != lastFilUseHeight || dfac != lastFilDiameter || lastCorrectNormals != SettingsProvider.Instance.ThreeDSettings.CorrectNorms || lastMachineType != SettingsProvider.Instance.SelectedPrinterSettings.printerType;
            lastFilHeight = h;
            lastFilWidth = w;
            lastFilDiameter = dfac;
            lastFilUseHeight = fixedH;
            lastMachineType = SettingsProvider.Instance.SelectedPrinterSettings.printerType;
            lastCorrectNormals = SettingsProvider.Instance.ThreeDSettings.CorrectNorms;
            for (int i = 0; i < MaxExtruder; i++)
            {
                col = SettingsProvider.Instance.SelectedPrinterSettings.Extruders[i].ExtruderColor;
                ///if (i == 1) col = SettingsProvider.Instance.ThreeDSettings.filament2;
                //else if (i == 2) col = SettingsProvider.Instance.ThreeDSettings.filament3;
                //else col = SettingsProvider.Instance.ThreeDSettings.filament;
            ...
            if (!SettingsProvider.Instance.ThreeDSettings.checkDisableTravelMoves)
                drawMoves();
            ...
            if (showSelection)
            {
                ...
                selectionStart = GCodePoint.toFileLine(editor.FileIndex, editor.selectedLine);
                selectionEnd = GCodePoint.toFileLine(editor.FileIndex, editor.selectedLine + editor.selectedLinesCount-1);
                ...
                col = SettingsProvider.Instance.ThreeDSettings.selectedFilament;
                defaultColor[0] = (float)col.R / 255.0f;
                defaultColor[1] = (float)col.G / 255.0f;
                defaultColor[2] = (float)col.B / 255.0f;
